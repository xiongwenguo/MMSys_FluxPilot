...
public:
    ...
    explicit RRMultiPathScheduler(TransportDownloadTaskInfo & tansDlTkInfo, 
            ...
            std::set<DataNumber>& alreadyDledQueue)
            : MultiPathSchedulerAlgo(tansDlTkInfo.m_rid, dlsessionmap, downloadQueue, lostPiecesQueue,alreadyDledQueue),
    ...
    void SortSession(std::multimap<Duration, fw::shared_ptr<SessionStreamController>>& sortmmap) override
    {
        ...
        for (auto&& sessionItor: m_dlsessionmap)
        {
            ...
            expectionRTT = expectionRTT + sessionItor.second->GetRtt() * (2.0 * sessionItor.second->GetLossRate());
            sortmmap.emplace(expectionRTT, sessionItor.second);
        }

    }
    void UpdateDataNumber(DataNumber pno)
    {
        m_alreadyDledQueue.insert(pno);
        for(auto it=m_alreadyDledQueue.begin();it!=m_alreadyDledQueue.end();)
        {
            if(m_continuousPktNum+1==*it)
            {
                m_continuousPktNum += 1;
        ...
        SPDLOG_DEBUG("m_continuousPktNum:{}",m_continuousPktNum);
    }

    bool toReInject(DataNumber pno)
    {
        bool haveSent=false;
        Timepoint now_t = Clock::GetClock()->Now();
        Timepoint sendtic = Clock::GetClock()->Now();
        Duration rtt = Duration::Zero();
        Duration dev = Duration::Zero();
        for(auto &&sessionItor : m_dlsessionmap)
        {
            
            if(sessionItor.second->FindPktInFlight(pno,sendtic))
            {
                haveSent = true;
                rtt = sessionItor.second->GetRtt();
                dev = sessionItor.second->GetStdDeviation();
                break;
            }
        }
        if(!haveSent)
        ...
        {
            if(now_t - sendtic > rtt + dev)
            {
                
                return true;
            }
            Duration expectAckTimeInterval = (rtt+dev)-(now_t-sendtic);
            Duration minRTT = GetMinRTT();
            if(minRTT < expectAckTimeInterval)
            {
                return true;
            }
            else
    ...
    Duration GetMinRTT()
    {
        Duration minRtt(Duration::FromSeconds(100));
        for (auto &&sessionItor : m_dlsessionmap)
        {
    ...
    void ReInject()
    {
        ...
        int64_t intervalInMilliSeconds = interval.ToMilliseconds();
        
        Duration maxRTT(Duration::FromSeconds(0));
        for (auto &&sessionItor : m_dlsessionmap)
        {
            auto RTT = sessionItor.second->GetRtt()+sessionItor.second->GetStdDeviation(); 
            if(RTT > maxRTT)
                maxRTT = RTT;           
        }
        SPDLOG_DEBUG("maxRTT:{} ms",maxRTT.ToMilliseconds());
        std::set<DataNumber> reInjectQueue;
        int64_t bufferTimeIntervalInMilliSeconds = maxRTT.ToMilliseconds()*2;
        if(intervalInMilliSeconds<bufferTimeIntervalInMilliSeconds*10)
        {
            // 起步阶段不进行ReInject
            return;
        }
        m_tansDlTkInfoRR.m_byterate = 1024 * 1024 / 8;
        //SPDLOG_DEBUG("byte rate:{}",m_tansDlTkInfoRR.m_byterate);
        //SPDLOG_DEBUG("interval from start to now:{} ms",intervalInMilliSeconds);
        //SPDLOG_DEBUG("buffer interval:{} ms",bufferTimeIntervalInMilliSeconds);
        int pktCntNeeded =1.0*m_tansDlTkInfoRR.m_byterate*((bufferTimeIntervalInMilliSeconds+intervalInMilliSeconds)/1000.0)/1024.0 + 1;
        //SPDLOG_DEBUG("pktCntNeeded:{}",pktCntNeeded);
        //SPDLOG_DEBUG("continuousOktCnt:{}",m_continuousPktNum);
        for(int32_t i=m_continuousPktNum+1;i<pktCntNeeded;i++)
        {
            reInjectQueue.insert(i);
        }
        for(auto it=reInjectQueue.begin();it!=reInjectQueue.end();it++)
        {
            if(!toReInject(*it))
            {
                reInjectQueue.erase(it);
            }
        }
        SortSession(m_sortmmap);
        for (auto&& rtt_sess: m_sortmmap)
        {
            ...
            int i=0;
            std::vector<DataNumber> requestVec;
            for(auto it=reInjectQueue.begin();it!=reInjectQueue.end()&&i<4;i++)
            {
                requestVec.push_back(*it);
                reInjectQueue.erase(it++);    
            }
            if(!sessStream->DoRequestdata(sessId,requestVec))
            {
                reInjectQueue.insert(requestVec.begin(),requestVec.end());
            }
        }
