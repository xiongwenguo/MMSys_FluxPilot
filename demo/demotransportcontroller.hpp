...
public:
    ...
    void OnPiecePktTimeout(const basefw::ID &peerid, const std::vector<std::pair<uint32_t, int32_t>> &spns) override;
...
private:
    ...
    std::set<std::pair<SeqNumber, DataNumber>> m_downloadPieces;                                             /// main task download queue
    std::set<std::pair<SeqNumber, DataNumber>> m_lostPiecesl;                                                /// lost packets will be stored here till retransmission
    std::set<std::pair<SeqNumber, DataNumber>> m_priorityPieces;
    std::set<std::pair<SeqNumber, DataNumber>> m_alreadyDlPieces;
