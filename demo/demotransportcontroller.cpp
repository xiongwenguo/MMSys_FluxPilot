        ...
        : MPDTransportController(ctlConfig)
{
    ...
    bicccConfig.maxCwnd = m_transCtlConfig->maxWnd;
    bicccConfig.minCwnd = m_transCtlConfig->minWnd;
    bicccConfig.beta = m_transCtlConfig->beta;
    bicccConfig.low_window = m_transCtlConfig->low_window;
    bicccConfig.Smax = m_transCtlConfig->Smax;
    bicccConfig.Smin = m_transCtlConfig->Smin;
    bicccConfig.Wmax = m_transCtlConfig->Wmax;
        ...
        std::weak_ptr<MPDTransCtlHandler> transCtlHandler)
{
    ...
    m_multipathscheduler.reset(
            new RRMultiPathScheduler(m_tansDlTkInfo, m_sessStreamCtlMap, m_downloadPieces, m_lostPiecesl, m_alreadyDlPieces));
...
void DemoTransportCtl::OnSessionCreate(const fw::ID& sessionid)
{
    ...
    if (sessionItor == m_sessStreamCtlMap.end())
    {
        ...
        m_sessStreamCtlMap[sessionid]->StartSessionStreamCtl(sessionid, bicccConfig, shared_from_this());
    }
    else
    {
