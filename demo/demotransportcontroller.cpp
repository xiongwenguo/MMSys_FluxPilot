 ...
 **/
bool DemoTransportCtl::StartTransportController(TransportDownloadTaskInfo tansDlTkInfo, std::weak_ptr<MPDTransCtlHandler> transCtlHandler)
{
    if (isRunning)
    {
    ...
    m_tansDlTkInfo = tansDlTkInfo;
    m_transctlHandler = transCtlHandler;
    /*
    RRMultiPathScheduler(const fw::ID &taskid,
                        std::map<fw::ID, fw::shared_ptr<SessionStreamController>> &dlsessionmap,
                        std::set<DataNumber> &downloadQueue, std::set<std::pair<SeqNumber, DataNumber>> &priorityQueue, 
                        std::set<std::pair<SeqNumber, DataNumber>> &lostPiecesQueue);
    */
    m_multipathscheduler.reset(
        new RRMultiPathScheduler(m_tansDlTkInfo, m_sessStreamCtlMap,
                                 m_downloadPieces, m_priorityPieces, m_lostPiecesl, m_alreadyDlPieces));
...
void DemoTransportCtl::OnPieceTaskAdding(std::vector<int32_t> &datapiecesVec)
{
    ...
    {
        auto rt = m_downloadPieces.emplace(std::make_pair(0,dataPiece));
        if (!rt.second)
        {
            // warning: already has
        }
...
void DemoTransportCtl::OnPiecePktTimeout(const basefw::ID &peerid, const std::vector<std::pair<uint32_t, int32_t>> &spns)
{
    if (!isRunning)
    {
        return;
