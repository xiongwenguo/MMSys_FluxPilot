...
public:
    ...
    void StartSessionStreamCtl(const basefw::ID& sessionId, BICCongestionCtlConfig& ccConfig,
            std::weak_ptr<SessionStreamCtlHandler> ssStreamHandler)
    {
        ...
        // cc
        m_ccConfig = ccConfig;
        m_congestionCtl.reset(new BICCongestionControl(m_ccConfig));
    ...
    {
        return m_inflightpktmap.FindPktInFlight(pno,sendtic);
    }
    ...
    Duration GetStdDeviation()
    {
        Duration d{ Duration::Zero() };
        if (isRunning)
        {
            d = m_rttstats.GetStandardOrMeanDeviation();
        }
        SPDLOG_TRACE("rtt = {}", d.ToDebuggingValue());
        return d;
    }
...
private:
    ...
    BICCongestionCtlConfig m_ccConfig;
